...
export const en = {
  ...
  codeRouteNone: 'None',
  codeRouteHint: 'Applies to an explicit code dispatch only. The run’s Lead and Peers inherit the route you selected in the chat. A DSH provider route cannot serve high-risk review or audit: the live provider catalog exposes no backend version, so use a host CLI for that path.',
...
export const zh: Record<OrcLocaleKey, string> = {
  ...
  codeRoute: '代码路由',
  codeRouteNone: '无',
  codeRouteHint: '仅适用于显式的代码调度。运行中的 Lead 与 Peer 继承你在对话中选择的路由。DSH 提供商路由无法用于高风险评审或审计：实时提供商目录不提供后端版本，该路径请使用主机 CLI。',
